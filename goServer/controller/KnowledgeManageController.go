...
func (k *KnowledgeManageController) Router(engine *gin.Engine) {
	...
	engine.DELETE("/Faq/:id/", k.DeleteFaq)
...
// DeleteFaq  删除Faq
func (k *KnowledgeManageController) DeleteFaq(ctx *gin.Context) {
	id := ctx.Param("id")
	faq := kn.Faq{
		Id: id,
	}
	...
	if faq.FaqType == 0 {
		utils.DbEngine.Delete(&faq)
		utils.Response(ctx, 200, "success", nil)
	}
	process := kn.Process{}
	utils.DbEngine.Where(kn.Process{FaqId: id}).Find(&process)
	utils.DbEngine.Where(kn.Transition{ProcessId: process.Id}).Delete(&kn.Transition{})
	utils.DbEngine.Where(kn.Node{ProcessId: process.Id}).Delete(&kn.Node{})
	utils.DbEngine.Delete(&process)
	utils.DbEngine.Delete(&faq)
	utils.Response(ctx, 200, "success", nil)
}
